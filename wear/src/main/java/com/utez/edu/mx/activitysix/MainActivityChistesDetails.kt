...
import com.utez.edu.mx.activitysix.databinding.ActivityMainChistesDetailsBinding
import com.utez.edu.mx.activitysix.models.note.Note
import com.utez.edu.mx.activitysix.models.note.NoteAdapter

class MainActivityChistesDetails : Activity() {

    private lateinit var binding: ActivityMainChistesDetailsBinding
    lateinit var adapter: NoteAdapter

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityMainChistesDetailsBinding.inflate(layoutInflater)
        setContentView(binding.root)
        // Firebase
        val db = Firebase.firestore
        val listNotes = ArrayList<Note>()
        val docRef = db.collection("jokes")
        docRef.get()
